 ...
 * 
 *  @(#) $Id: sample1n.c 1011 2016-07-11 02:20:01Z coas-nagasima $
 */
...
net_syslog (uint_t prio, const char *format, ...)
{
	...
	if (connected) {
		...
		while ((c = *format++) != '\0' && i < TNUM_LOGPAR) {
			...
			switch (c) {
			case 'd':
				log.logpar[i++] = (intptr_t)va_arg(ap, int_t);
				break;
			case 'u':
			case 'x':
			case 'X':
				log.logpar[i++] = (intptr_t)va_arg(ap, uint_t);
				break;
			case 'p':
				log.logpar[i++] = (intptr_t)va_arg(ap, void *);
				break;
			case 'c':
				log.logpar[i++] = (intptr_t)va_arg(ap, int_t);
				break;
			case 's':
				log.logpar[i++] = (intptr_t)va_arg(ap, const char *);
				break;
			case '\0':
				format--;
				break;
