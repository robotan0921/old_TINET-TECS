...
const TINIB _kernel_tinib_table[TNUM_STSKID] = {
	...
	{ (TA_ACT), (intptr_t)(1), (TASK)(if_mbed_phy_task), INT_PRIORITY(IF_MBED_PHY_PRIORITY), ROUND_STK_T(IF_MBED_PHY_STACK_SIZE), _kernel_stack_IF_MBED_PHY_TASK },
	{ (TA_NULL), (intptr_t)(0), (TASK)(net_timer_task), INT_PRIORITY(NET_TIMER_PRIORITY), ROUND_STK_T(NET_TIMER_STACK_SIZE), _kernel_stack_NET_TIMER_TASK },
	{ (TA_HLNG), (intptr_t)(0), (TASK)(ether_output_task), INT_PRIORITY(ETHER_OUTPUT_PRIORITY), ROUND_STK_T(ETHER_OUTPUT_STACK_SIZE), _kernel_stack_ETHER_OUTPUT_TASK },
	{ (TA_HLNG|TA_ACT), (intptr_t)(0), (TASK)(ether_input_task), INT_PRIORITY(ETHER_INPUT_PRIORITY), ROUND_STK_T(ETHER_INPUT_STACK_SIZE), _kernel_stack_ETHER_INPUT_TASK },
	{ (TA_HLNG), (intptr_t)(0), (TASK)(tcp_output_task), INT_PRIORITY(TCP_OUT_TASK_PRIORITY), ROUND_STK_T(TCP_OUT_TASK_STACK_SIZE), _kernel_stack_TCP_OUTPUT_TASK }
};

TINIB _kernel_atinib_table[3];

TCB _kernel_tcb_table[TNUM_TSKID];

const ID _kernel_torder_table[TNUM_STSKID] = { 
	TSKID_tTask_LogTask_Task, TASK1, TASK2, MAIN_TASK, EXC_TASK, IF_MBED_PHY_TASK, NET_TIMER_TASK, ETHER_OUTPUT_TASK, ETHER_INPUT_TASK, TCP_OUTPUT_TASK
};
...
static MPF_T _kernel_mpf_MPF_NET_BUF_REASSM[NUM_MPF_NET_BUF4_REASSM * COUNT_MPF_T(sizeof(T_NET_BUF4_REASSM))];
static MPFMB _kernel_mpfmb_MPF_NET_BUF_REASSM[NUM_MPF_NET_BUF4_REASSM];
const MPFINIB _kernel_mpfinib_table[TNUM_SMPFID] = {
	...
	{ (TA_TFIFO), (NUM_MPF_NET_BUF4_REASSM), ROUND_MPF_T(sizeof(T_NET_BUF4_REASSM)), _kernel_mpf_MPF_NET_BUF_REASSM, _kernel_mpfmb_MPF_NET_BUF_REASSM }
};
...
ALMINIB _kernel_aalminib_table[1];

T_NFYINFO _kernel_aalm_nfyinfo_table[1];
...
const uint_t _kernel_tnum_isr_queue = 3;

const ISR_ENTRY _kernel_isr_queue_list[3] = {
	{ 37, &(_kernel_isr_queue_table[0]) },
	{ 231, &(_kernel_isr_queue_table[1]) },
	{ 232, &(_kernel_isr_queue_table[2]) }
};

QUEUE _kernel_isr_queue_table[3];
...
const ISRINIB _kernel_isrinib_table[TNUM_SISRID] = {
	{ (TA_NULL), (intptr_t)(&tISR_CB_tab[0]), (&(_kernel_isr_queue_table[1])), (ISR)(tISR_start), (1) },
	{ (TA_NULL), (intptr_t)(&tISR_CB_tab[1]), (&(_kernel_isr_queue_table[2])), (ISR)(tISR_start), (1) },
	{ (TA_NULL), (intptr_t)(0), (&(_kernel_isr_queue_table[0])), (ISR)(intno1_isr), (1) }
};
...
const ID _kernel_isrorder_table[TNUM_SISRID] = { 
	ISRID_tISR_SIOPortTarget1_RxISRInstance, ISRID_tISR_SIOPortTarget1_TxISRInstance, INTNO1_ISR
};
...
const size_t _kernel_kmmsz = TOPPERS_ROUND_SZ(KMM_SIZE, sizeof(MB_T));
MB_T *const _kernel_kmm = _kernel_memory;
