 ...
 * 
 *  @(#) $Id: ethernet.c 1011 2016-07-11 02:20:01Z coas-nagasima $
 */
 ...
 */
void ether_set_link_up(T_IFNET *ether)
{
  if (!(ether->flags & IF_FLAG_LINK_UP)) {
    ether->flags |= IF_FLAG_LINK_UP;

    if (ether->flags & IF_FLAG_UP) {
#if LWIP_ARP
      /* For Ethernet network interfaces, we would like to send a "gratuitous ARP" */ 
      if (ether->flags & IF_FLAG_ETHARP) {
        etharp_gratuitous(ether);
      }
#endif /* LWIP_ARP */
    }
    if (ether->link_callback) {
      (ether->link_callback)(ether);
    }
  }
}
 ...
 */
void ether_set_link_down(T_IFNET *ether)
{
  if (ether->flags & IF_FLAG_LINK_UP) {
    ether->flags &= ~IF_FLAG_LINK_UP;
    if (ether->link_callback) {
